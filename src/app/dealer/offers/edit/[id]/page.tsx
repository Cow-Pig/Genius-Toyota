...
function computePreview(values: OfferFormValues) {
  ...
  if (offerType === 'lease') {
    const payment = calculateLeasePayment(
      msrp,
      (residualPercentage ?? 60) / 100,
      termMonths,
      moneyFactor ?? 0.0025,
    );
    const total = payment + (fees ?? 0) / Math.max(termMonths, 1);
    return {
      payment: total,
      narrative: `Using ${formatPercent((residualPercentage ?? 60) / 100)} residual and ${(mileageAllowance ?? 12000).toLocaleString()} mi/year.`,
    };
  }
  ...
  return {
    payment,
    narrative: `Includes ${(apr ?? 6).toFixed(2)}% APR after ${formatCurrency(dueAtSigning ?? 0)} due at signing.`,
  };
}
